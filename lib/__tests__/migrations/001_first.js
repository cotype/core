...
async function default_1(ctx) {
    await ctx.rewrite(content_migrations_test_1.initialModel.name, (data, meta) => {
        if (meta.published) {
            return { ...data, initialField: content_migrations_test_1.publishedData.initialField };
        }
        else if (meta.latest) {
            return { ...data, initialField: content_migrations_test_1.draftsData.initialField };
        }
        return data;
    });
}
