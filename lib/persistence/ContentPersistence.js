...
class ContentPersistence {
    ...
    applyPreHooks(event, model, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!preHook)
                return data;
            try {
                const hookData = yield preHook(model, data);
                return hookData;
            }
            catch (error) {
    ...
    applyPostHooks(event, model, dataRecord) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!postHook)
                return;
            try {
                yield postHook(model, dataRecord);
            }
            catch (error) {
    ...
    create(principal, model, data, models) {
        return __awaiter(this, void 0, void 0, function* () {
            data = this.setOrderPosition(data, model, models);
            const hookData = yield this.applyPreHooks("onCreate", model, data);
            const { storeData, searchData } = yield this.splitStoreAndIndexData(hookData, model);
            // NOTE: principal.id will always be set since anonymous access is prevented by ACL.
            const id = yield this.adapter.create(storeData, searchData, model, models, principal.id);
            this.applyPostHooks("onCreate", model, { id, data: storeData });
            return { id: String(id), data: storeData };
        });
    }
    createRevision(principal, model, id, data, models) {
        return __awaiter(this, void 0, void 0, function* () {
            const { storeData, searchData } = yield this.splitStoreAndIndexData(data, model);
            // NOTE: principal.id will always be set since anonymous access is prevented by ACL.
            const rev = yield this.adapter.createRevision(storeData, searchData, model, models, id, principal.id);
            return { rev, data: storeData };
        });
    }
    fetchRefs(ids, contentFormat, previewOpts = {}, join = {}, model) {
        return __awaiter(this, void 0, void 0, function* () {
            // load all content the loaded content is referencing
            const contentRefs = yield this.adapter.loadContentReferences(ids, model, this.models, previewOpts, (0, filterRefData_1.getDeepJoins)(join, this.models));
            // load meta data for media file for this content and all the references
            const mediaRefs = yield this.adapter.loadMediaFromContents(ids.concat(contentRefs.map(c => c.id)), previewOpts.publishedOnly);
            ...
            Object.entries(sortedContentRefs).forEach(([type, items]) => {
                ...
                return Object.values(items).forEach(item => {
                    return Object.assign(Object.assign({}, item), { data: (0, convert_1.default)({
    ...
    load(principal, model, id, join = {}, contentFormat, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const content = yield this.adapter.load(model, id, previewOpts);
            if (!content)
                return content;
            const refs = yield this.fetchRefs([id], contentFormat, previewOpts, join, model);
    ...
    loadInternal(principal, model, id, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const content = yield this.adapter.load(model, id, previewOpts);
            if (content) {
    ...
    loadItem(principal, model, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const content = yield this.adapter.load(model, id);
            if (content) {
    ...
    loadRevision(principal, model, id, rev) {
        return __awaiter(this, void 0, void 0, function* () {
            const content = yield this.adapter.loadRevision(model, id, rev);
            if (content) {
    ...
    update(principal, model, id, data, models) {
        return __awaiter(this, void 0, void 0, function* () {
            const hookData = yield this.applyPreHooks("onSave", model, data);
            const rev = yield this.createRevision(principal, model, id, hookData, models);
            const resp = {
                id: String(id),
                data: rev.data
            };
    ...
    schedule(principal, model, id, schedule) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.adapter
    ...
    publishRevision(principal, model, id, rev, models) {
        return __awaiter(this, void 0, void 0, function* () {
            const resp = yield this.adapter
    ...
    list(principal, model, opts, criteria) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!opts.orderBy) {
            ...
            const { total, items } = yield this.adapter.list(model, this.models, opts, criteria);
            return {
                total,
    ...
    findByMedia(media) {
        return __awaiter(this, void 0, void 0, function* () {
            const contents = yield this.adapter.findByMedia(media);
            return this.createItems(contents);
        });
    }
    find(principal, model, opts, contentFormat, join, criteria, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const items = yield this.adapter.list(model, this.models, opts, criteria, previewOpts);
            if (!items.total)
                return Object.assign(Object.assign({}, items), { _refs: { content: {}, media: {} } });
            const _refs = yield this.fetchRefs(items.items.map(i => i.id), contentFormat, previewOpts, join, model);
            const convertedItems = Object.assign(Object.assign({}, items), { items: items.items.map(i => (Object.assign(Object.assign({}, i), { data: (0, convert_1.default)({
    ...
    findInternal(principal, model, opts, criteria, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.adapter.list(model, this.models, opts, criteria, previewOpts);
        });
    }
    search(principal, term, exact, opts, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const { total, items } = yield this.adapter.search(term, exact, opts, previewOpts);
            return {
                total,
                items: items
    ...
    externalSearch(principal, term, opts, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const clearedTerm = term.replace("*", " ");
            const textSearch = yield this.adapter.search(clearedTerm.length > 0 ? clearedTerm : " ", false, opts, previewOpts);
    ...
    suggest(principal, term, previewOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            const { items } = yield this.adapter.search(term, true, {}, previewOpts);
    ...
    rewrite(modelName, iterator) {
        ...
        return this.adapter.rewrite(model, this.models, (data, meta) => __awaiter(this, void 0, void 0, function* () {
            let rewritten = yield iterator(data, meta);
            if (rewritten) {
                rewritten = yield this.applyPreHooks("onSave", model, rewritten);
    ...
    migrate(migrations) {
        this.adapter.migrate(migrations, (adapter, outstanding) => __awaiter(this, void 0, void 0, function* () {
    ...
    listLastUpdatedContent(principal, opts = { limit: 50, offset: 0 }, byUser) {
        return __awaiter(this, void 0, void 0, function* () {
            const filteredModels = this.models.filter((0, filterModels_1.createModelFilter)(principal));
            const listChunk = yield this.adapter.listLastUpdatedContent(filteredModels.map(m => m.name), opts, byUser ? principal.id : undefined);
            return {
    ...
    listUnpublishedContent(principal, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const filteredModels = this.models.filter((0, filterModels_1.createModelFilter)(principal));
            const listChunk = yield this.adapter.listUnpublishedContent(filteredModels.map(m => m.name), opts);
            return {
                total: listChunk.total,
                items: this.createItemsWithAuthorAndDate(listChunk)
            };
    ...
    setOrderPosition(data, model, models) {
        return __awaiter(this, void 0, void 0, function* () {
            if (model.orderBy) {
                const lastItem = yield this.adapter.list(model, models, {
                    limit: 1,
