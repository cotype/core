...
class KnexContent {
    ...
    create(storeData, indexData, model, models, author) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.testUniqueFields(model, models, storeData);
            const [id] = yield this.knex("contents")
                .insert({
                type: model.name
            })
                .returning("id");
            storeData = yield this.testPositionFields(model, models, storeData, id);
            yield this.createRev(storeData, indexData, model, models, id, 1, author);
            return id;
        });
    }
    /**
     ...
     * already exists.
     */
    testUniqueFields(model, models, data, id) {
        return __awaiter(this, void 0, void 0, function* () {
            let nonUniqueFields = [];
            const uniqueFields = (0, getAlwaysUniqueFields_1.default)(model, true);
            if (uniqueFields) {
                const resp = yield Promise.all(uniqueFields.map((f) => __awaiter(this, void 0, void 0, function* () {
                    const criteria = {};
                    const value = f
                        .split(".")
                    ...
                    const res = yield Promise.all([p1, p2]);
     ...
     */
    testPositionFields(model, models, data, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const positionFields = (0, getPositionFields_1.getPositionFieldsWithValue)(data, model);
            if (positionFields.length > 0) {
                yield Promise.all(positionFields.map(({ fieldPath, value }) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const items = yield this.list(model, models, opts, criteria, {
                        ignoreSchedule: true
                    });
                    if (items.items[0]) {
    ...
    createRevision(storeData, indexData, model, models, id, author) {
        return __awaiter(this, void 0, void 0, function* () {
            storeData = yield this.testPositionFields(model, models, storeData, id);
            yield this.testUniqueFields(model, models, storeData, id);
            const [content] = yield this.knex("contents")
                .select(["latest_rev"])
                .where({ id });
    ...
    createRev(data, searchData, model, models, id, rev, author) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.knex("content_revisions").insert({
                id,
                rev,
            ...
            yield this.knex("contents").where({ id }).update({ latest_rev: rev });
            yield this.indexRevision(data, searchData, model, models, id, rev);
            return rev;
        });
    }
    indexRevision(data, searchData, model, models, id, rev, published = false) {
        return __awaiter(this, void 0, void 0, function* () {
            // Delete content_values except the published ones
            yield this.knex("content_values").where({ id, published }).del();
            // Delete content_search except the published one
            yield this.knex("content_search").where({ id, published }).del();
            yield this.extractValues(data, model, id, rev, published);
            yield this.extractText(searchData, model, id, rev, published);
            ...
            if (refs.length) {
                ...
                for (const refKey in refs) {
                    if (refs.hasOwnProperty(refKey)) {
                        ...
                        try {
                            yield this.knex("content_references").insert(Object.assign({ id,
                                rev }, ref));
                        }
                        catch (error) {
    ...
    loadContentReferences(id, model, models, previewOpts = {}, joins = [{}]) {
        return __awaiter(this, void 0, void 0, function* () {
            let fullData = [];
            ...
            const fetch = (ids, types, prevTypes, first) => __awaiter(this, void 0, void 0, function* () {
                ...
                const inverseRefs = hasRefs
                    ...
                    : [];
                const [data, inverseData] = yield Promise.all([refs, inverseRefs]);
                if (inverseData) {
                    return data
                        .concat(inverseData)
            ...
            for (let i = 0; i < joins.length + 1; i++) {
                const join = joins[i];
                const data = yield fetch(checkIds, Object.keys(join || {}), Object.keys(joins[i - 1] || {}), i === 0);
    ...
    loadMediaFromContents(ids, published) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    load(model, id, previewOpts = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const content = yield k
                .select(["contents.*", "content_revisions.data"])
                .first();
            return content ? this.parseData(content, model) : null;
        });
    }
    loadRevision(model, id, rev) {
        return __awaiter(this, void 0, void 0, function* () {
            const content = yield this.knex("content_revisions as cr")
    ...
    listVersions(model, id) {
        return __awaiter(this, void 0, void 0, function* () {
            const versions = yield this.knex("content_revisions as cr")
    ...
    setPublishedRev(model, id, publishedRev, models) {
        return __awaiter(this, void 0, void 0, function* () {
            if (publishedRev === null) {
                // Check if content is referenced by another content
                yield this.checkReferrers(id);
            }
            // TODO: check if referenced content still exists
            // Delete values from previously published revision
            yield this.knex("content_values").where({ id, published: true }).del();
            // Delete search from previously published revision
            yield this.knex("content_search").where({ id, published: true }).del();
            if (publishedRev !== null) {
                // Insert values for newly published revision
                const { data } = yield this.loadRevision(model, id, publishedRev);
                const c = yield this.knex("contents").where({ id }).first();
                yield this.checkReferences(id, publishedRev, c);
                yield this.extractValues(data, model, id, publishedRev, true);
                yield this.extractText(data, model, id, publishedRev, true);
            }
            // Update the published_rev column
            yield this.knex("contents")
     ...
     */
    checkReferences(id, rev, schedule) {
        return __awaiter(this, void 0, void 0, function* () {
            const { visibleFrom, visibleUntil } = schedule;
            const references = yield this.knex
    ...
    checkReferrers(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const contents = yield this.knex
                .distinct(["contents.id", "contents.type", "content_revisions.data"])
                .from("content_references")
                .join("contents", join => {
    ...
    delete(model, id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.knex("contents")
                ...
                .update({ deleted: true });
            yield this.knex("content_references").where({ id }).del();
        });
    }
    schedule(model, id, schedule) {
        return __awaiter(this, void 0, void 0, function* () {
            const { visibleFrom = null, visibleUntil = null } = schedule;
            const c = yield this.knex("contents")
                .where({ type: model.name, id })
                .first();
            const rev = c.published_rev || c.latest_rev;
            yield this.checkReferrers(id);
            yield this.checkReferences(id, rev, schedule);
            yield this.knex("contents")
                .where({ type: model.name, id })
                .update({
    ...
    search(term, exact, listOpts, previewOpts = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            let k;
            let text = term || "";
            ...
            const [count] = yield k.clone().countDistinct(`${searchTable}.id as total`);
    ...
    list(model, models, listOpts = {}, criteria = {}, previewOpts = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [count] = yield k.clone().countDistinct("contents.id as total");
            const total = Number(count.total);
            if (total === 0)
                return { total, items: [] };
            ...
            return {
                total,
                items: (yield items).map((item) => this.parseData(item, model))
            };
        });
    }
    rewrite(model, models, iterator) {
        return __awaiter(this, void 0, void 0, function* () {
            const revs = yield this.knex("content_revisions as cr")
                .join("contents as c", "cr.id", "c.id")
                .where({
                "c.type": model.name
            })
            ...
            for (const { id, rev, latest_rev, published_rev } of revs) {
                const { data } = yield this.knex("content_revisions")
                    .where({ id, rev })
                    .first("data");
                const meta = {
                    id,
                ...
                const rewritten = yield iterator(JSON.parse(data), meta);
                if (rewritten) {
                    const { storeData, searchData } = rewritten;
                    yield this.knex("content_revisions")
                        .update("data", JSON.stringify(storeData))
                        .where({ id, rev });
                    if (meta.latest || meta.published) {
                        yield this.indexRevision(storeData, searchData, model, models, id, rev, rev === published_rev);
                    }
                }
            }
        });
    }
    migrate(migrations, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            const applied = yield this.knex("content_migrations").where({
                state: "applied"
            });
            ...
            try {
                ...
                yield this.knex("content_migrations").insert(names.map(name => ({ name, state: "pending" })));
                const tx = yield this.knex.transaction(); // TODO: Fix Line
                try {
                    yield callback(new KnexContent(tx), outstanding);
                    yield tx.commit();
                    // Mark migrations as 'applied'
                    yield this.knex("content_migrations")
                    ...
                    yield this.knex("content_migrations").whereIn("name", names).del();
                }
            }
            catch (err) {
                ...
                return new Promise((resolve, reject) => {
                    const poll = (retries = 3 * 60) => __awaiter(this, void 0, void 0, function* () {
                        // Wait until there are no more pending migrations
                        const [pending] = yield this.knex("content_migrations")
                            .where({ state: "pending" })
                            .count("* as count");
    ...
    listLastUpdatedContent(models, opts, user) {
        return __awaiter(this, void 0, void 0, function* () {
            const k = this.knex("contents")
            ...
            const [count] = yield k.clone().count("contents.id as total");
            const total = Number(count.total);
            if (total === 0)
                return { total, items: [] };
            k.select([
    ...
    listUnpublishedContent(models, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const k = this.knex("contents")
                .join("content_revisions as cr", join => {
                join.on(`contents.id`, "cr.id");
