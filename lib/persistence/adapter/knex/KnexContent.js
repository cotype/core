...
export default class KnexContent {
    ...
    async testUniqueFields(model, models, data, id) {
        ...
        if (nonUniqueFields.length)
            throw new UniqueFieldError(nonUniqueFields);
    }
    /**
                            ...
                            if (stringPath === fieldPath) {
                                if (!value) {
                                    ...
                                    if (items.items[1]) {
                                        ...
                                        visit(items.items[1].data, model, {
                                            position(nextPosition, g, h, nextStringPath) {
                                                if (nextStringPath === fieldPath) {
                                                    data = setPosition(data, model, value, nextPosition, true, fieldPath);
                                                }
                                            }
                                        });
                                    }
                                        ...
                                        data = setPosition(data, model, value, "z", true, fieldPath);
                                    }
                                }
                            }
                        }
        ...
        Object.entries(values).forEach(([field, value]) => {
            ...
            if (Array.isArray(value)) {
                return value.forEach(val => rows.push(Object.assign({ id,
                    rev,
                    published,
                    field }, serialize(val, fieldType))));
            }
            return rows.push(Object.assign({ id,
                rev,
                published,
                field }, serialize(value, fieldType)));
        });
                    ...
                    k.innerJoin("content_values as vals" + counter, join => {
                        ...
                        Object.entries(criterion).forEach(([op, value]) => {
                            ...
                            if (isComparable(field)) {
                                ...
                                k.andWhere(`vals${counter}.numeric`, sqlOp, toNumber(value, field));
                            }
            ...
            refs.forEach(ref => {
                ...
                fields.forEach(f => {
                    if (f.model === _content && fieldNames.includes(f.fieldName)) {
                        _update(parsedData, f.path, val => (val || []).concat({
                            _ref: "content",
                            _content,
                            _id: String(_id)
                        }));
