...
describe.each(implementations)("%s adapter", (_, impl) => {
    ...
    describe("content", () => {
        ...
        const createNews = (...data) => {
            return Promise.all(data.map(d => content.create({ ...sampleNews, ...d }, { ...sampleNews, ...d }, news, models.content, author)));
        };
        const samplePage = {
            title: "Sample page"
        };
        const createPages = (...data) => {
            return Promise.all(data.map(d => content.create({ ...samplePage, ...d }, { ...samplePage, ...d }, pages, models.content, author)));
        };
        const createIndexContent = (...data) => {
            return Promise.all(data.map(d => content.create({ ...d }, { ...d }, indexContent, models.content, author)));
        };
        const createPositionContent = (...data) => {
            return Promise.all(data.map(d => content.create({ ...d }, { ...d }, positionContent, models.content, author)));
        };
        beforeAll(async () => {
