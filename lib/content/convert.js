...
export default function convert({ content, contentRefs, contentModel, allModels, contentFormat = "html", mediaUrl, previewOpts = {} }) {
    /**
    ...
    const convertReferences = (ref, field) => {
        if (ref) {
            ...
            convertedRef._url = getRefUrl((allRefData || {}).data, referencedModel.urlPath);
            return convertedRef;
        }
    };
    visit(content, contentModel, {
        richtext(delta) {
            ...
            if (contentFormat)
                return formatQuillDelta(delta, contentFormat);
        },
        list(list) {
