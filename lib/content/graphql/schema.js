...
import log from "../../log";
import { GraphQLSchema, GraphQLString, GraphQLBoolean, GraphQLFloat, GraphQLObjectType, GraphQLList, GraphQLUnionType } from "graphql";
const scalars = {
...
export function buildSchema(models) {
    ...
    function createType(def, name) {
        ...
        if (def.type === "object") {
            return new GraphQLObjectType({
                name,
                fields: () => _.mapValues(def.fields, (def2, prop) => ({
        ...
        if (def.type === "union") {
            return new GraphQLUnionType({
                name,
                types: Object.values(() => _.mapValues(def.types, (def2, prop) => ({
                    type: createType(def2, name + _.upperFirst(prop))
                })))
            });
        }
    ...
    types = _.zipObject(_.map(models.content, "name"), models.content.map(({ name, fields: f }) => {
        const def = { type: "object", fields: f };
        return createType(def, name);
    }));
    const fields = _.mapValues(types, type => ({
        type,
        args: {
