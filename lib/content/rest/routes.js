...
export default function routes(router, persistence, models, externalDataSources, mediaUrl, responseHeaders) {
    ...
    modes.forEach(mode => {
        ...
        models
            ...
            router.get(`/rest/${mode}/${type}`, async (req, res) => {
                ...
                if (model.collection === "singleton") {
                    ...
                    if (result.total > 0) {
                        ...
                        if (fields) {
                            result = pickFieldsFromResultData(result, fields);
                        }
                        const { items, _refs } = result;
                        const [item] = items;
                        return res.json(Object.assign(Object.assign({}, item.data), { _id: item.id.toString(), _refs: filterRefData(items, _refs, join, models) }));
                    }
                    res.status(404).end();
                }
                else {
                    ...
                    if (fields) {
                        results = pickFieldsFromResultData(results, fields);
                    }
            ...
            router.get(`/rest/${mode}/${type}/:id`, async (req, res) => {
                ...
                res.json(Object.assign(Object.assign({}, result.data), { _id: result.id.toString(), _refs: filterRefData([result], result._refs, join, models) }));
            });
            // loadByUniqueField
            if (model.uniqueFields) {
                model.uniqueFields.forEach(uniqueField => {
                    router.get(`/rest/${mode}/${type}/${uniqueField}/:uniqueValue`, async (req, res) => {
                        ...
                        const { items, _refs } = result;
                        res.json(Object.assign(Object.assign({}, items[0].data), { _id: items[0].id.toString(), _refs: filterRefData(items, _refs, join, models) }));
                    });
                });
            }
        });
