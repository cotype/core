...
function routes(router, persistence, models, externalDataSources, mediaUrl, responseHeaders) {
    ...
    modes.forEach(mode => {
        ...
        /** Search */
        router.get(`/rest/${mode}/search/content`, (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            const results = yield content.externalSearch(principal, term, {
                limit,
                offset,
                models: searchModels
            }, req.previewOpts);
            const preparedResults = (0, prepareSearchResults_1.default)(results, models, mediaUrl);
            const imageData = yield media.load(principal, preparedResults.mediaIds);
            const mediaObj = {};
            imageData.forEach(m => {
                mediaObj[m.id] = m;
            });
        ...
        router.get(`/rest/${mode}/search/suggest`, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const { principal, query } = req;
            const { term } = query;
            const searchModels = getSearchModels(query);
            if (!searchModels.length) {
            ...
            const results = yield content.suggest(principal, term, req.previewOpts);
            res.json(results);
        }));
        models
            ...
            // List
            router.get(`/rest/${mode}/${type}`, (req, res) => __awaiter(this, void 0, void 0, function* () {
                const { principal, query } = req;
                const _a = query, { search = {}, limit = 50, offset = 0, join, order, orderBy, fields } = _a, rest = __rest(_a, ["search", "limit", "offset", "join", "order", "orderBy", "fields"]);
                ...
                if (model.collection === "singleton") {
                    let result = yield dataSource.find(principal, model, opts, format, join, criteria, req.previewOpts);
                    if (result.total > 0) {
                        ...
                        const [item] = items;
                        return res.json(Object.assign(Object.assign({}, item.data), { _id: item.id.toString(), _refs: (0, filterRefData_1.default)(items, _refs, join, models) }));
                    }
                    res.status(404).end();
                }
                else {
                    let results = yield dataSource.find(principal, model, opts, format, join, criteria, req.previewOpts);
                    if (fields) {
                    ...
                    res.json({
                        total,
                        items: items.map(i => (Object.assign({ _id: i.id.toString() }, i.data))),
                        _refs: (0, filterRefData_1.default)(items, _refs, join, models)
                    });
                }
            }));
            // load
            router.get(`/rest/${mode}/${type}/:id`, (req, res) => __awaiter(this, void 0, void 0, function* () {
                const { principal, params, query } = req;
                const { join, fields } = query;
                checkPermissionToJoin(req.principal, join);
                const dataSource = getDataSource(type);
                const format = req.get("x-richtext-format");
                let result = yield dataSource.load(principal, model, params.id, join, format, req.previewOpts);
                if (!result)
                ...
                res.json(Object.assign(Object.assign({}, result.data), { _id: result.id.toString(), _refs: (0, filterRefData_1.default)([result], result._refs, join, models) }));
            }));
            // loadByUniqueField
            if (model.uniqueFields) {
                model.uniqueFields.forEach(uniqueField => {
                    router.get(`/rest/${mode}/${type}/${uniqueField}/:uniqueValue`, (req, res) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        const format = req.get("x-richtext-format") || "html";
                        const dataSource = getDataSource(type);
                        let result = yield dataSource.find(principal, model, { limit: 1, offset: 0 }, format, join, criteria, req.previewOpts);
                        if (!result.total)
                            return res.status(404).end();
                        if (fields) {
                            result = (0, pickFieldsFromResultData_1.default)(result, fields);
                        ...
                        const { items, _refs } = result;
                        res.json(Object.assign(Object.assign({}, items[0].data), { _id: items[0].id.toString(), _refs: (0, filterRefData_1.default)(items, _refs, join, models) }));
                    }));
                });
            }
        });
