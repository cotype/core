...
function routes(router, persistence, storage) {
    ...
    router.post("/admin/rest/upload", uploadHandler, (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        for (const fileKey in filesUpload) {
            if (filesUpload.hasOwnProperty(fileKey)) {
                ...
                const { width, height, ext, hash, mime } = yield (0, inspect_1.default)(storage.retrieve(id), storage.getFile(id));
                let mimetype = mime;
                ...
                try {
                    yield media.create(principal, file);
                    files.push(file);
                }
                catch (error) {
                    // Find the already existing file(s)
                    const [data] = yield media.findByHash([hash]);
    ...
    router.get("/admin/rest/media", (req, res) => __awaiter(this, void 0, void 0, function* () {
        const { principal, query } = req;
        const { limit = 50, offset = 0, orderBy, order, search, mimetype, unUsed, used } = query;
        const list = yield media.list(principal, {
            limit,
            offset,
            orderBy,
            order,
    ...
    router.get("/admin/rest/media/*", (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        const [data] = yield media.load(principal, [id]);
        if (!data)
            return res.status(404).end();
        res.json(data);
    }));
    router.post("/admin/rest/media/*", (req, res) => __awaiter(this, void 0, void 0, function* () {
        const { principal, params, body } = req;
        const id = prepareMediaId(params[0]);
        const data = yield media.update(principal, id, body);
        if (data) {
            res.status(200).end();
        }
        else {
    ...
    router.delete("/admin/rest/media/*", (req, res) => __awaiter(this, void 0, void 0, function* () {
        const { principal, params } = req;
        const id = prepareMediaId(params[0]);
        try {
            yield media.delete(principal, id);
            storage.remove(id);
            res.status(204).end();
        }
        catch (err) {
