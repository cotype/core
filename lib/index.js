...
export * from "./utils";
export { ContentPersistence, MigrationContext, log };
...
export const clientMiddleware = (basePath = "/") => process.env.DEVCLIENT // Use Proxy to Dev Server
    ? [
    ...
    : promiseRouter()
        .use(urlJoin(basePath, "/admin"), express.static(root, {
        maxAge: "1y",
        immutable: true,
...
export async function getRestApiBuilder(opts) {
    const { basePath } = opts;
...
export async function init(opts) {
    ...
    const persistence = await createPersistence(models, await opts.persistenceAdapter, {
    ...
    const media = Media(persistence, models, storage, thumbnailProvider, baseURLS.cms);
    const app = express();
    ...
    router.use("/admin/rest/docs", swaggerUi(urlJoin(baseURLS.cms, "admin/rest/docs/"), urlJoin(baseURLS.cms, "admin/rest/swagger.json")));
    router.get("/admin/rest", (req, res) => res.redirect(urlJoin(baseURLS.cms, "admin/rest/docs")));
    ...
    app.get(baseURLS.cms, (_, res) => res.redirect(urlJoin(baseURLS.cms, "admin")));
