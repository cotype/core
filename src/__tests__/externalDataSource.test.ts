...
const newsModel = models.find(({ name }) => name === "news") as Model;
const pagesModel = models.find(({ name }) => name === "pages") as Model;
const productsModel = models.find(({ name }) => name === "products") as Model;
...
const newsSource: ReadOnlyDataSource = {
  contentTypes: ["news"],
  list: jest.fn(),
  load: jest.fn(),
...
const pagesSource: WritableDataSource = {
  contentTypes: ["pages"],
  list: jest.fn(),
  load: jest.fn(),
  loadInternal: jest.fn(),
...
const productsSource: VersionedDataSource = {
...
describe("external data source support", () => {
  ...
  describe("with ReadOnlyDataSource", () => {
    it("reads a list", async () => {
      const response = {
        total: 5000,
        items: [{ foo: "bar" } as any, { baz: "brrt" } as any]
      };
    ...
    it("reads a single item", async () => {
      ...
      await expect(newsSource.loadInternal).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(newsModel),
        someID
      );
  ...
  describe("with WritableDataSource", () => {
    it("creates an item", async () => {
      ...
      await expect(pagesSource.create).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
    ...
    it("deletes an item", async () => {
      ...
      await expect(pagesSource.delete).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(pagesModel),
        id
      );
    ...
    it("updates an item", async () => {
      ...
      await expect(res.body).toEqual(response);
      await expect(pagesSource.update).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(pagesModel),
        id,
  ...
  describe("with VersionedDataSource", () => {
    it("lists versions", async () => {
      ...
      await expect(productsSource.listVersions).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(productsModel),
        id
      );
    ...
    it("receives a single reversion", async () => {
      ...
      await expect(productsSource.loadRevision).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(productsModel),
        id,
        rev
    ...
    it("publishes a specific revision", async () => {
      ...
      await expect(productsSource.publishRevision).toHaveBeenCalledWith(
        expect.objectContaining(adminUser),
        expect.objectContaining(productsModel),
        id,
        rev,
