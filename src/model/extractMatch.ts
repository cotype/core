...
function bestMatch(sentences: string[], query: string) {
...
export default function extractMatch(obj: object, model: Model, query: string) {
  ...
  visit(obj, model, {
    ...
    number(n: number, field: NumberType) {
      if (field.search !== false) add(String(n));
    },
  ...
  return (bestMatch(sentences, query) || sentences)[0];
}
