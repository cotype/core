...
export default class KnexContent implements ContentAdapter {
  ...
  async createRev(
    ...
    await this.indexRevision(storeData, indexData, model, models, id, rev);
  ...
  async indexRevision(
    ...
    await this.extractValues(storeData, model, id, rev, published);
    await this.extractText(indexData, model, id, rev, published);
  ...
  async rewrite(
    ...
    for (const { id, rev, latest_rev, published_rev } of revs) {
      ...
      if (rewritten) {
        const { storeData, indexData } = rewritten;
